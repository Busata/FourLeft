...
public class DiscordAutoPostingService {
    ...
    public void syncResults(String clubId) {
        ...
        configurations.stream().filter(DiscordClubConfiguration::isAutopostingEnabled).forEach(config -> {
            checkAutoposting(clubId, config.getChannelId());
        });
        configurations.stream().filter(DiscordClubConfiguration::isAutopostingDisabled).forEach(config -> {
            saveEntries(clubId, config.getChannelId());
        });
    }

    private void saveEntries(String clubId, Long channelId) {
        this.clubService.getActiveEvent(clubId).ifPresent(event -> {
            ...
            List<AutopostEntry> list = entries.stream().map(entry -> {
                return new AutopostEntry(event.getId(), channelId, -1L, entry.getPlayerKey());
            }).toList();
    ...
    private void checkAutoposting(String clubId, Long channelId) {

        this.clubService.getActiveEvent(clubId).ifPresent(event -> {
            String leaderboardId = event.getLeaderboardId();

            List<ClubLeaderboardEntry> entries = clubLeaderboardService.findEntries(leaderboardId);
            List<AutopostEntry> postedEntries = autopostingEntryService.findPostedEntries(event.getId(), channelId);

            if(entries.size() == postedEntries.size()) {
                return;
            } else {
                syncPosts(channelId, event, entries, postedEntries);
            }
        });
    }

    private void syncPosts(Long channelId, Event event, List<ClubLeaderboardEntry> newEntries, List<AutopostEntry> postedEntries) {

        tryReusingMessage(channelId, postedEntries).ifPresentOrElse(message -> {
            editMessage(channelId, message.id(), event, newEntries, postedEntries);
        }, () -> {
            createNewMessage(channelId, event, newEntries, postedEntries);
        });

    }

    private List<ClubLeaderboardEntry> findToBePosted(Long messageId, List<ClubLeaderboardEntry> newEntries, List<AutopostEntry> postedEntries) {
        Set<String> postedIds = postedEntries.stream().map(AutopostEntry::getPlayerKey).collect(Collectors.toSet());

        List<ClubLeaderboardEntry> unpostedEntries   = newEntries.stream().filter(newEntry -> !postedIds.contains(newEntry.getPlayerKey())).toList();
    ...
    private Optional<DiscordMessageTo> tryReusingMessage(Long channelId, List<AutopostEntry> postedEntries) {
        return discordGateway.getLastChannelMessage(channelId).filter(message -> {
    ...
    private void createNewMessage(Long channelId, Event event, List<ClubLeaderboardEntry> newEntries, List<AutopostEntry> postedEntries) {
        List<String> postedIds = postedEntries.stream().map(AutopostEntry::getPlayerKey).collect(Collectors.toList());
        List<ClubLeaderboardEntry> toBePosted   = newEntries.stream().filter(newEntry -> !postedIds.contains(newEntry.getPlayerKey())).limit(ENTRIES_LIMIT).sorted(Comparator.comparing(ClubLeaderboardEntry::getRank)).toList();

        publisher.publishEvent(new AutoPostNewMessageEvent(channelId, new AutoPostMessageSummary(event, newEntries.size(), toBePosted)));
    }

    private void editMessage(Long channelId, Long messageId, Event event, List<ClubLeaderboardEntry> newEntries, List<AutopostEntry> postedEntries) {
        List<ClubLeaderboardEntry> toBePosted = findToBePosted(messageId, newEntries, postedEntries);
        publisher.publishEvent(new AutoPostEditMessageEvent(channelId, messageId, new AutoPostMessageSummary(event,newEntries.size(), toBePosted)));
