...
public class JobDispatcher {
    ...
    /** When a target of this type/ref should next run, derived from its state. */
    public LocalDateTime nextRunAt(JobType type, String ref) {
        JobHandler handler = handlers.get(type);
        return handler != null ? handler.nextRunAt(ref) : ApplicationClock.now();
    }
}
