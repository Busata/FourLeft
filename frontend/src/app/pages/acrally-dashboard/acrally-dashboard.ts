 ...
 * add an entry here plus a matching child route to grow the section. The admin "Users" tab
 * is appended only for administrators (the route's adminGuard is the real gate).
 */
@Component({
...
export class AcrallyDashboard {
  ...
  readonly tabs = computed(() => {
    ...
    if (this.auth.isAdmin()) {
      tabs.push({ path: 'users', label: 'Users' });
    }
    return tabs;
  });
}
