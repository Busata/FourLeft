...
/**
 * The unbanded "top X%" standing to one significant figure, e.g. P1 of 1234 → "Top 0.08%". Used on
 * the profile table in place of the coarse {@link percentileBand}. `toPrecision(1)` round-trips
 * through Number to drop any exponential form (100 → "1e+2" → "100"). Blank when rank/field unknown.
 */
export function percentileExact(rank: number | null, total: number | null | undefined): string {
  ...
  const pct = (rank / total) * 100;
  return `Top ${String(Number(pct.toPrecision(1)))}%`;
}
