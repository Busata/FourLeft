...
- Base URL comes from `api_base` in the agent's `config.toml`
  (e.g. `api_base = "http://127.0.0.1:8799"`). Trailing slashes are stripped;
...
while driving         POST /sessions/{id}/heartbeat   (every heartbeat_secs, default 1 s)
...
- **abort** — the run ended without a usable result. Reasons the agent sends:
  ...
  - `"no-result"` — the stage finished but the save file never produced a
    result within `save_wait_secs` (default 20 s).
...
| `driver_id`     | string        | no       | Stable club-member id from config; omitted if unset |
| `club_id`       | string        | no       | Club id from config; omitted if unset            |
...
Sent while driving, at most once per `heartbeat_secs` (default 1 s).
...
|------------------|----------------|-------------------------------------------------|
| `api_base`       | (required)     | Base URL all endpoint paths are appended to      |
| `api_key`        | unset          | Enables `Authorization: Bearer` header           |
| `driver_id`      | unset          | Included in session-start and result payloads    |
| `club_id`        | unset          | Included in session-start and result payloads    |
| `heartbeat_secs` | `1.0`          | Minimum interval between heartbeats              |
| `save_wait_secs` | `20.0`         | How long a finish waits for a save-file result before aborting with `"no-result"` |
