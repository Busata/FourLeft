...
public class PowerStageCommand implements BotCommandOptionHandler {
    @Override
    ...
    public String getOption() {
        return CommandOptions.POWER_STAGE;
    }
    ...
    public ImmutableApplicationCommandOptionData buildOption() {
        return ApplicationCommandOptionData.builder()
                .name(getOption())
                .description("Get results for the powerstage of the event")
    ...
    public Mono<Message> createResults(ChatInputInteractionEvent event) {
        try {
            return event.createFollowup(InteractionFollowupCreateSpec.builder().addAllEmbeds(resultsFetcher.getPowerstageResults(event.getInteraction().getChannelId())).build());
        } catch (Exception ex) {
