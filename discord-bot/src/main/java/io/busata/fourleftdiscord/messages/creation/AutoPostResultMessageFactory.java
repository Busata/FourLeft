...
public class AutoPostResultMessageFactory {
    ...
    MessageTemplate NORMAL_TEMPLATE = BASE.copy()
            .withRecurringTemplate(
                    "entries",
                    "**${rank}** • **${nationalityEmoticon}** • **${name}** • ${totalTime} *(${totalDiff})* • *${vehicleName}*");

    MessageTemplate POWERSTAGE_TEMPLATE = BASE.copy()
            .withRecurringTemplate(
                    "entries",
                    "${powerStageBadge}**${rank}** • **${nationalityEmoticon}** • **${name}** • ${totalTime} *(${totalDiff})* • *${vehicleName}*");

    MessageTemplate RANKED_BADGE_TEMPLATE = BASE.copy()
            .withRecurringTemplate(
                    "entries",
                    "*${badgeRank}* **${rank}** • **${nationalityEmoticon}** • **${name}** • ${totalTime} *(${totalDiff})* • *${vehicleName}*");

    MessageTemplate POWERSTAGE_AND_RANKED_BADGE_TEMPLATE = BASE.copy()
            .withRecurringTemplate(
                    "entries",
                    "${powerStageBadge}*${badgeRank}* **${rank}** • **${nationalityEmoticon}** • **${name}** • ${totalTime} *(${totalDiff})* • *${vehicleName}*");
